...
public:
    ...
    static bool CheckForWhiteSpace( const char* p )
    {
        return IsWhiteSpace( *p ) != 0;
    }
...
static bool IsWhiteSpace(char p)
{
    ...
    // if (c == '\t')  // exclude tab (0x09)
    //     return false;

    return !IsUTF8Continuation(p) && isspace(c);
}
...
public:
    ...
    //modifications for epub parsing:
    bool hasPrecedingWhitespace = false; //store wether a whitespace has collapsed before the start of this element

...
void StrPair::CollapseWhitespace()
{
    ...
    // ⭐ Detect if original string had any whitespace at all
    bool hadWhitespace = false;
    for (const char* t = _start; *t; ++t) {
        if (XMLUtil::IsWhiteSpace(*t)) {
            hadWhitespace = true;
            break;
        }
    }
    ...
    if (*_start) {
        const char* p = _start;   // read pointer
        char* q = _start;         // write pointer

        while (*p) {
            if (XMLUtil::IsWhiteSpace(*p)) {
                p = XMLUtil::SkipWhiteSpace(p, 0);
                if (*p == 0) {
                    break;  // trims trailing space
                }
                *q = ' ';
                ++q;
            }
    ...
    // ⭐ If everything collapsed away but whitespace existed → keep single space
    if (*_start == 0 && hadWhitespace) {
        *_start = ' ';
        _start[1] = 0;
    }
}
...
char* XMLDocument::Identify( char* p, XMLNode** node, bool first )
{
    ...
    bool precedingWhiteSpace = XMLUtil::CheckForWhiteSpace(p);
    ...
    if ( XMLUtil::StringEqual( p, xmlHeader, xmlHeaderLen ) ) {
        ...
        if (WhitespaceMode() == PEDANTIC_WHITESPACE && first && p != start && *(p + elementHeaderLen) == '/') {
            ...
            returnNode->_parseLineNum = _parseCurLineNum;
            returnNode->hasPrecedingWhitespace = precedingWhiteSpace;
            p += elementHeaderLen;
        }
    }
    else {
